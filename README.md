...
## 1-1. 📌 주제 선정 이유
### 👀 문제 인식
최근 급격한 사회경제 상태변화와 의료기기 및 기술의 발달로 인해 보건의료서비스에 많은 변화가 일어나고 있다.
이와 같이 급변하는 의료환경에서 병원은 환자 확보 및 관리를 위해 고객(환자)의 이탈 방지가 가장 문제시되는 것으로 판단된다.
따라서 환자의 성별이나 나이, 질병의 종류나 응급실 이용 여부 등에 따른 이탈 확률을 계산하여 환자 관리의 효율을 높이고자 한다.
